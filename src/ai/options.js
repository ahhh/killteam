...
function movementPlans(controller, ctx) {
  ...
  const build = (dest, branch, rationale) => {
  ...
  const shelter = controller._bestBy(dests, (d) =>
    controller._cover(state, op, d.x, d.y, enemies) * 2 -
    controller._exposure(state, op, d.x, d.y, enemies));
  ...
  const samePlace = press && shelter &&
    Math.hypot(press.x - shelter.x, press.y - shelter.y) < 1;
  if (!samePlace) {
    const coverPlan = build(shelter, 'cover', [
      `Moves ${shelter?.length.toFixed(1) ?? '0'}" into the best cover in reach and holds Conceal`,
    ]);
...
function titleFor(state, op, plan, branch) {
  ...
  switch (branch) {
    ...
    case 'cover':
      return `Break ${(plan.estimate?.moved ?? 0).toFixed(1)}" into cover`;
    case 'withdraw':
...
export function buildTacticOptions(controller, ctx, plans, { count = 3 } = {}) {
  ...
  // The best VALID plan of each branch. Validation is the expensive step (it
  // re-traces sight lines at engine fidelity), so it only runs on a plan that
  // is about to become the front-runner for its branch.
  const best = new Map();
  const rest = [];
  for (const plan of pool) {
    const branch = branchOf(plan);
    if (best.has(branch)) { rest.push(plan); continue; }
    if (!controller._planIsValid(state, op, plan, enemies, tactics)) continue;
    best.set(branch, plan);
  }
